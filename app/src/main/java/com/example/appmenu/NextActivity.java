...
public class NextActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Bundle myBundle;
        try {
            myBundle = myIntent.getBundleExtra("Sum");
            Double stn = myBundle.getDouble("stn");
            Double sth = myBundle.getDouble("sth");
            textView.setText("Sum: " + (stn + sth));

        }catch (Exception e) {
            Toast.makeText(NextActivity.this, "Error Sum",Toast.LENGTH_SHORT).show();
        }
        try {

                myBundle = myIntent.getBundleExtra("hello");
                String name = myBundle.getString("name");
                int age = myBundle.getInt("age");
                textView.setText("Name: " + name + "\nAge: " + age);

            }catch (Exception e){
                Toast.makeText(NextActivity.this, "Error Name & Age",Toast.LENGTH_SHORT).show();
            }
